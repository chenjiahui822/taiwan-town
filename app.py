"""
台湾小镇业态评估系统 - 完全基于 Plotly 版（中文完美显示）
"""
...
# 标题（中文正常显示）
st.title("🎮 台湾小镇业态评估系统")
st.caption("统计分析 + 互动玩法 | 业态PK · 匹配测试 · 游客模拟器 · 自定义推荐")
                ...
                if '年龄' in df.columns:
                    ...
                    for shop in shop_types:
                        ...
                        if len(groups) >= 2:
                            ...
                            results.append(
                                {'业态': shop, 'F值': round(f_stat, 3), 'p值': round(p_val, 4), '显著性': sig})
                    if results:
                ...
                if '带小孩' in df.columns:
                    ...
                    for shop in shop_types:
                        ...
                        if len(group1) > 1 and len(group2) > 1:
                            ...
                            sig = '***' if p_val < 0.001 else (
                                '**' if p_val < 0.01 else ('*' if p_val < 0.05 else 'ns'))
                            results.append({'业态': shop, '带小孩均值': round(group1.mean(), 3),
...
st.divider()
st.caption("© 平潭文旅课题组 | 基于 Plotly 的中文完美显示版")